...
export async function POST(req: Request) {
  try {
    ...
    if (String(password).length < 8) return fail("A senha precisa ter pelo menos 8 caracteres.");
    ...
    await prisma.transaction.create({
      data: { userId: user.id, accountId: account.id, creditor: "Abertura de conta", type: "entrada", value: seed.balance, status: "concluído", date: new Date().toLocaleDateString("pt-BR") }
    });
    await prisma.auditLog.create({ data: { userId: user.id, action: "REGISTER", details: { email: cleanEmail, cpf: cpf ? "provided" : "not_provided" } } });
    const token = createToken(user.id);
    const res = NextResponse.json({ user: { id: user.id, name: user.fullName, email: user.email } }, { status: 201 });
